...
if m == "CIFAR":
    ...
    z_dim = 128
    data_loader = dg.generate_MNIST_dataloader()
...
try:
    generator.load_state_dict(torch.load(f"{parameters_load_path+m}_GANs_G.pth"))
    discriminator.load_state_dict(torch.load(f"{parameters_load_path+m}_GANs_D.pth"))
except:
...
def train_gans():
    ...
    for epoch in range(epoches):
        ...
        print(f"\td_loss:{sum_d_loss/len_data}")
        print(f"\tg_loss: {sum_g_loss/(t*len_data)}")   
