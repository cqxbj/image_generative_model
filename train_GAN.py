...
generator, discriminator = my_F.load_gans_model(model_name, z_dim=z_dim, n_class=n_class)
...
if start_training:
    ...
    for epoch in range(epoches):
        ...
        for x, labels in data_loader:
            ...
            with torch.no_grad():
                fake_images = generator(noises, labels).to(device)
        ...
        epoch_g_loss = sum_g_loss.item()/(t*len_data)
        ...
        if epoch % freq_gen_sample == 0:
            my_F.gans_save_samples(generator, epoch=epoch,input_str="DAAI COMP7015 XIANGBOJIE")
            my_F.plot_values(d_losses,g_losses,labels=["Discriminator losses", "Generator losses"], model_name="gans")
